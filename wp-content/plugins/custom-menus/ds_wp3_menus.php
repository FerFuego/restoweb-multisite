...
Plugin URI: 
Version: 1
Description: Wordpress Multisite Network plugin to toggle more of the administration menus in the same way "Plugins" is already toggled. Go to Network-->Settings->Menu Settings to "Enable administration menus". All menus are unchecked and disabled by default, except when logged in as Network Admin.
Author: 
Author URI: 
Network: true
*/
...
//------------------------------------------------------------------------//
	function __construct() {
		...
		if( !isset($menu_perms[ 'app_cus' ]) ) {
			add_action( 'admin_init', array($this, 'ds_customize'), 10 );
			add_filter( 'init', array($this,'ds_removecustmizer_metacap'), 10 );
		}
...
//------------------------------------------------------------------------//
	...
	function ds_customize() {
		remove_action( 'plugins_loaded', '_wp_customize_include', 10);
		remove_action( 'admin_enqueue_scripts', '_wp_customize_loader_settings', 11);
		add_action( 'load-customize.php', array($this, 'ds_override_load_customizer_action') );
	...
	function ds_removecustmizer_metacap() {
		add_filter( 'map_meta_cap', array($this,'ds_removecustmizer_metacap_filter'), 10, 4 );
	}	
	
	function ds_removecustmizer_metacap_filter( $caps = array(), $cap = '', $user_id = 0, $args = array() ) {
		if ($cap == 'customize' ) {
			return array( 'nope' );	//thanx wp-crap
		}
		return $caps;
	}
	...
	function ds_reduce_favorite_actions ($wp_toolbar) {
		...
		if( !isset( $menu_perms[ 'menu_users' ] ) && current_user_can('read') ) {
			$wp_toolbar->remove_node( 'users' );
		}
	}
	...
	function ds_mu_menu_options() {
		$menu_items = array( 
			...
			'jetpack_modules'	=> __('Jetpack Settings'),
			...
			'app_back'		=> __('Appearance Background'),
			'plugins' 			=> __( 'Plugins' ),
	...
	function ds_menu_option() {
		echo '<div class="d-block ml-5"><a href="/wp-admin/options-general.php?page=menu_editor&sub_section=editor" class="button button-primary ml-5" target="_blank">Acceder a Admin Menu Editor</a></div>';
	}
}
new CM_Menus();	
?>