...
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("No se puede llamar a una clase como una función"); } }
...
var Html5Qrcode = /*#__PURE__*/function () {
  ...
  function Html5Qrcode(elementId, verbose) {
    ...
    if (!qrcode) {
      throw 'qrcode no está definido, utilice el minified/html5-qrcode.min.js' + ' para un apoyo adecuado';
    }
  ...
  _createClass(Html5Qrcode, [{
    ...
    value: function start(cameraId, configuration, qrCodeSuccessCallback, qrCodeErrorCallback) {
      ...
      if (!cameraId) {
        throw "cameraId es requerido";
      }

      if (!qrCodeSuccessCallback || typeof qrCodeSuccessCallback != "function") {
        throw "qrCodeSuccessCallback es requerido y debe ser una función.";
      }
      ...
      if (isShadedBoxEnabled) {
        ...
        if (qrboxSize < Html5Qrcode.MIN_QR_BOX_SIZE) {
          throw "el tamanio minimo de 'config.qrbox' es" + " ".concat(Html5Qrcode.MIN_QR_BOX_SIZE, "px.");
        }

        if (qrboxSize > width) {
          throw "'config.qrbox' no debe ser mayor que el " + "el ancho del elemento HTML.";
        }
      ...
      var setupUi = function setupUi(width, height) {
        ...
        if (qrboxSize > height) {
          console.warn("[Html5Qrcode] config.qrboxsize es mayor " + "que la altura del video. El sombreado será ignorado");
        }
      ...
      var foreverScan = function foreverScan() {
        ...
        if ($this._localMediaStream) {
          ...
          try {
            ...
            qrCodeErrorCallback("Error de análisis de código QR, error = ".concat(exception));
          }
        }
      ...
      return new Promise(function (resolve, reject) {
        if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
          ...
          navigator.mediaDevices.getUserMedia({
            ...
            reject("Error al obtener userMedia, error = ".concat(err));
          });
          ...
          reject("Transmisión de la cámara web no compatible con el navegador.");
        }
      });
    }
    /**
    ...
    showImage) {
      ...
      if (!imageFile || !(imageFile instanceof File)) {
        throw "el argumento imageFile es obligatorio y debe ser instancia " + "de File. Use 'event.target.files[0]'";
      }

      showImage = showImage === undefined ? true : showImage;

      if ($this._isScanning) {
        throw "Cierre el análisis en curso antes de escanear un archivo.";
      }
      ...
      return new Promise(function (resolve, reject) {
        ...
        inputImage.onload = function () {
          ...
          try {
            ...
            reject("Error de análisis de código QR, error = ".concat(exception));
          }
        };
    ...
    value: function _clearElement() {
      if (this._isScanning) {
        throw 'No se puede borrar mientras la exploración está en curso, ciérrela primero.';
      }
    ...
    value: function _getShadedRegionBounds(width, height, qrboxSize) {
      if (qrboxSize > width || qrboxSize > height) {
        throw "'config.qrbox' no debe ser mayor que el " + "ancho y alto del elemento HTML.";
      }
    ...
    value: function _createShadedElement(height, qrRegion, shadingPosition) {
      ...
      switch (shadingPosition) {
        ...
        default:
          throw "No soportado shadingPosition";
      }
    ...
    value: function _insertShaderBorders(shaderElem, qrRegion, shadingPosition) {
      ...
      var insertBorder = function insertBorder(top, left) {
        if (!(top !== null && left !== null)) {
          throw "Los sombreadores deben tener posiciones definidas";
        }
        ...
        if (navigator.mediaDevices && navigator.mediaDevices.enumerateDevices && navigator.mediaDevices.getUserMedia) {
          ...
          navigator.mediaDevices.getUserMedia({
            ...
            stream.oninactive = function (_) {
              return _this3._log("Todas las corrientes cerradas");
            };
            ...
            navigator.mediaDevices.enumerateDevices().then(function (devices) {
              ...
              _this3._log("".concat(results.length, " resultado encontrado"));
          ...
          _this3._log("no puede consultar los dispositivos compatibles.");

          reject("no puede consultar los dispositivos compatibles.");
        }
      });
    }
  }, {
  ...
  _createClass(Html5QrcodeScanner, [{
    ...
    value: function render(qrCodeSuccessCallback, qrCodeErrorCallback) {
      ...
      this.qrCodeSuccessCallback = function (message) {
        $this.__setStatus("ENCONTRADO", Html5QrcodeScanner.STATUS_SUCCESS);
        ...
        if (qrCodeSuccessCallback) {
          ...
          $this.__setHeaderMessage("Ultimo encontrado: ".concat(message), Html5QrcodeScanner.STATUS_SUCCESS);
        }
      }; // Add wrapper to failure callback


      this.qrCodeErrorCallback = function (error) {
        $this.__setStatus("ESCANEANDO");
      ...
      if (this.html5Qrcode) {
        if (this.html5Qrcode._isScanning()) {
          this.html5Qrcode.stop().then(function (_) {
            ...
            if ($this.verbose) {
              console.error("No se puede detener el escáner qrcode", error);
            }
    ...
    value: function __createHeader(dashboard) {
      ...
      titleSpan.innerHTML = "Escaner de código QR";
    ...
    value: function __createSectionControlPanel() {
      ...
      requestPermissionButton.addEventListener("click", function () {
        ...
        $this.__setStatus("PERMISO");

        $this.__setHeaderMessage("Solicitando permisos de cámara...");
        ...
        Html5Qrcode.getCameras().then(function (cameras) {
          ...
          if (!cameras || cameras.length == 0) {
            $this.__setStatus("No Cámaras", Html5QrcodeScanner.STATUS_WARNING);
          } else {
    ...
    value: function __renderCameraSelection(cameras) {
      ...
      cameraSelectionContainer.innerHTML = "Seleccione Cámara (".concat(cameras.length, ") &nbsp;");
      ...
      cameraActionStartButton.innerHTML = "Comenzar Escaneo";
      cameraActionContainer.appendChild(cameraActionStartButton);
      var cameraActionStopButton = document.createElement("button");
      cameraActionStopButton.innerHTML = "Detener Escaneo";
    ...
    value: function __createSectionSwap() {
      ...
      var TEXT_IF_FILE_SCAN_SELECTED = "Escanear usando la cámara directamente";
      var section = document.getElementById(this.__getDashboardSectionId());
      ...
      swithToFileBasedLink.addEventListener('click', function () {
        if (!$this.sectionSwapAllowed) {
          if ($this.verbose) {
            console.error("Intercambio de sección llamado cuando no está permitido");
          }
